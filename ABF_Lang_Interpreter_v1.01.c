...
BOOL chkEndian(void); /* check endianness, returns TRUE if system is little endian */
...
unsigned nameProc(int cmdPos) { /* get name and store it to nameBuf */
...
int condProc(int cmdPos, int condNum) { /* set int type conditions, returns condition count */
    ...
    for (i = 0; i < condNum; i++) {
        if (!(*cp)) return i;
            ...
            if (*cp == ',') x = addr;
            else if (!isUnsigned && *cp != ',') x = atoi(cp);
            else if (isUnsigned && *cp != ',') x = atou(cp);
            condCnt++;
        }
        else if (i == 1) {
            if (*cp == ',') y = addr;
            else if (!isUnsigned && *cp != ',') y = atoi(cp);
            else if (isUnsigned && *cp != ',') y = atou(cp);
            condCnt++;
        }
        else if (i == 2) {
...
void pAddrSet(int pos) { /* set pointers except for char types to mem + addr */
    if (pos == USE_CURR_ADDR) {
        ...
        if (ptrMode == 0) pc = (signed char*)(mem + pos);
...
int main(void) {
    ...
    initGlobalVar();
    puts("Ascii BrainFuck Language Interpreter Prompt v1.01");
...
void help(void) {
    ...
    puts("This program is licensed under Apache License Version 2.0");
    printf("Copyright 2020. Lee Geon-goo");
}
