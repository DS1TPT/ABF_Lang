...
#include <termios.h> /* for implementation of _getch() for Linux */
linuxGetche() {
    int ch;
    struct termios old;
...
3. find logic errors
4. check for ';' in loop and if statements
?. debug
...
cmdProc() { /* Interpret commands */
    ...
    for (i = 0; cmdBuf[i] != NULL; i++) {
        ...
        case 'g':
            mem[addr] = _getche();
            if (mem[addr] == 13 || mem[addr] == 10) printf("\n");
            break;
        case 'h':
            if (!isExec) {
        ...
        case 'r':
            ...
            if (j) {
                printf("?Error position number %u not found, cmd at %d, line %u", basicPosBuf, i, uLineCnt);
                return 1;
            }
            break;
        case 's':
...
void help() {
    ...
    puts("r: Return(jump) to pos pointed by pos buffer <: decrement pointer");
    ...
    puts("y: save current pos to pos buffer            \": set unsigned type");
    puts("z: Zerofill memory                           \\: print as int");
    puts("|: bitwise OR                                {: condition *(p+X) < *(p+Y)");
    puts("~: bitwise NOT                               }: condition *(p+X) > *(p+Y)");
    puts("For syntax & command usage, please refer to Github page.");
