...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        clrscr();
        puts("Ascii BrainFuck Language Interpreter Prompt v1.12");
        ...
        if (!szCmp(argv[1], "help")) {
            puts("ASCII BrainFuck(ABF) Language Interpreter, version 1.12");
            puts("To run a program without using prompt...");
            puts("[Interpreter file name] [program file name]");
            puts("Windows example: abf_lang_intrpr.exe c:\\abf\\program.abf");
            puts("*nix example: ./abf_lang_intrpr /abf/program.abf");
            puts("To run interpreter in prompt mode...");
            puts("[Interpreter file name]");
            puts("No argument should be given except for the file name of interpreter in order to run interpreter in prompt mode");
            exit(0);
        }
        pFile = fopen(argv[1], "r");
        if (pFile == NULL) {
            printf("?Could not load file %s\n", argv[1]);
            exit(10); /* 10: Could not load file */
        }
        else {
            rtnVal = exeFile();
            if (rtnVal == 1) {
        ...
        printf("?Too many arguments(%d). Give [help] as an argument to read help message.\n", argc - 1);
        exit(1); /* 1: Argument error */
    }
    /* the codes below must not be executed. */
    if (pFile) fclose(pFile);
    if (pLib) fclose(pLib);
    if (pRec) fclose(pRec);
    puts("!FATAL ERROR, CODE THAT SHOULD NOT BE EXECUTED HAS BEEN RUN!\n");
    exit(-100);
...
void help(void) {
    ...
    puts("Running a program without using prompt");
    puts("[Interpreter file name] [program file name]");
    puts("Windows example: abf_lang_intrpr.exe c:\\abf\\program.abf");
    puts("*nix example: ./abf_lang_intrpr /abf/program.abf");
